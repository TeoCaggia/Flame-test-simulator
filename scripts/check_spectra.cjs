...
let width=1000;
const nodes={};
const document={getElementById(id){return nodes[id]??=new Node(id);},createElementNS(ns,tag){return new Node(tag);},createElement(tag){return new Node(tag);}};
...
assert(source.includes("?'-30deg':'0deg'"));
...
assert(source.includes('startSpectrumMorph(previousElement,nextElement)'));
...
vm.runInContext(source.slice(0,stop)+`globalThis.qa={componentSamples,componentValueAt,speciesLabel,spectralColor,drawSpectrum,adjacentElementSymbol,morphSamples(from,to,progress){return spectrumMorphSamples({fromElement:elements.get(from),toElement:elements.get(to),cache:new Map()},960,bounds,progress);},setElement(symbol,reveal=1){current=elements.get(symbol)||null;spectrumReveal=reveal;drawSpectrum();},setView(a,b){spectrumView=[a,b];drawSpectrum();}};})();`,context);
const qa=context.qa;
  ...
  for(const element of data.elements){
    for(const reveal of [0,.5,1]){
      ...
      for(const trace of traces){
        ...
        assert(coords.every(([x,y])=>x>=20-1e-9&&x<=width-20+1e-9&&y>=36-1e-9&&y<=133+1e-9));
      ...
      assert(Math.abs(Math.min(...allCoords.map(p=>p[1]))-(133-97*reveal))<1e-9);
    }
  }
  qa.setElement(null);
...
qa.setView(380,770);
{
  ...
  const guides=descend(svg).filter(n=>n.tag==='line'&&n.attrs.stroke==='#aeb4bf');
...
{
  const svg=nodes.spectrum;
  const cursorLabel=descend(svg).find(n=>n.tag==='text'&&n.attrs.y==='22');
